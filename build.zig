...
fn addLinuxSystemCxxRuntime(b: *std.Build, root_module: *std.Build.Module) void {
    ...
    while (lines.next()) |raw_line| {
        ...
        // Zig injects its libc headers as system paths. Keep libstdc++ in the
        // earlier -I group so #include_next can reach Zig's C headers.
        root_module.addIncludePath(.{ .cwd_relative = line });
